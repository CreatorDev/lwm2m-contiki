/*
  ...
  gengetopt -i cmds/create_object.cmdline --string-parser --include-getopt --no-handle-error --no-handle-help --no-version --func-name=create_object_parser --arg-struct-name=create_object_args --file-name=cmds/create_object-cmdline 
...
const char *create_object_args_purpose = "";

const char *create_object_args_usage = "Usage: create_object [OPTIONS]...";

const char *create_object_args_versiontext = "";

const char *create_object_args_description = "";

const char *create_object_args_help[] = {
...
typedef enum {ARG_NO
  , ARG_INT
} create_object_parser_arg_type;

static
void clear_given (struct create_object_args *args_info);
static
void clear_args (struct create_object_args *args_info);

static int
create_object_parser_internal (int argc, char **argv, struct create_object_args *args_info,
                        struct create_object_parser_params *params, const char *additional_error);

static int
create_object_parser_required2 (struct create_object_args *args_info, const char *prog_name, const char *additional_error);
struct line_list
{
  char * string_arg;
...
static
void init_args_info(struct create_object_args *args_info)
{


  args_info->help_help = create_object_args_help[0] ;
  args_info->objectID_help = create_object_args_help[1] ;
  args_info->instanceID_help = create_object_args_help[2] ;
  
}

void
create_object_parser_print_version (void)
{
  printf ("%s %s\n",
     (strlen(CREATE_OBJECT_PARSER_PACKAGE_NAME) ? CREATE_OBJECT_PARSER_PACKAGE_NAME : CREATE_OBJECT_PARSER_PACKAGE),
     CREATE_OBJECT_PARSER_VERSION);

  if (strlen(create_object_args_versiontext) > 0)
    printf("\n%s\n", create_object_args_versiontext);
}

static void print_help_common(void) {
  create_object_parser_print_version ();

  if (strlen(create_object_args_purpose) > 0)
    printf("\n%s\n", create_object_args_purpose);

  if (strlen(create_object_args_usage) > 0)
    printf("\n%s\n", create_object_args_usage);

  printf("\n");

  if (strlen(create_object_args_description) > 0)
    printf("%s\n\n", create_object_args_description);
}

void
create_object_parser_print_help (void)
{
  int i = 0;
  print_help_common();
  while (create_object_args_help[i])
    printf("%s\n", create_object_args_help[i++]);
}

void
create_object_parser_init (struct create_object_args *args_info)
{
...
void
create_object_parser_params_init(struct create_object_parser_params *params)
{
  if (params)
    { 
      params->override = 0;
...
struct create_object_parser_params *
create_object_parser_params_create(void)
{
  struct create_object_parser_params *params = 
    (struct create_object_parser_params *)malloc(sizeof(struct create_object_parser_params));
  create_object_parser_params_init(params);  
  return params;
}
...
static void
create_object_parser_release (struct create_object_args *args_info)
{
...
int
create_object_parser_dump(FILE *outfile, struct create_object_args *args_info)
{
  int i = 0;

  if (!outfile)
    {
      fprintf (stderr, "%s: cannot dump options to stream\n", CREATE_OBJECT_PARSER_PACKAGE);
      return EXIT_FAILURE;
    }
...
int
create_object_parser_file_save(const char *filename, struct create_object_args *args_info)
{
  FILE *outfile;
  int i = 0;
  ...
  if (!outfile)
    {
      fprintf (stderr, "%s: cannot open file for writing: %s\n", CREATE_OBJECT_PARSER_PACKAGE, filename);
      return EXIT_FAILURE;
    }

  i = create_object_parser_dump(outfile, args_info);
  fclose (outfile);
...
void
create_object_parser_free (struct create_object_args *args_info)
{
  create_object_parser_release (args_info);
}
...
int
create_object_parser (int argc, char **argv, struct create_object_args *args_info)
{
  return create_object_parser2 (argc, argv, args_info, 0, 1, 1);
}

int
create_object_parser_ext (int argc, char **argv, struct create_object_args *args_info,
                   struct create_object_parser_params *params)
{
  int result;
  result = create_object_parser_internal (argc, argv, args_info, params, 0);

  return result;
}

int
create_object_parser2 (int argc, char **argv, struct create_object_args *args_info, int override, int initialize, int check_required)
{
  int result;
  struct create_object_parser_params params;
...
int
create_object_parser_required (struct create_object_args *args_info, const char *prog_name)
{
  int result = EXIT_SUCCESS;

  if (create_object_parser_required2(args_info, prog_name, 0) > 0)
    result = EXIT_FAILURE;
 ...
 * @param check_ambiguity @see create_object_parser_params.check_ambiguity
 * @param override @see create_object_parser_params.override
...
int update_arg(void *field, char **orig_field,
               ...
               create_object_parser_arg_type arg_type,
...
int
create_object_parser_internal (
  int argc, char **argv, struct create_object_args *args_info,
                        struct create_object_parser_params *params, const char *additional_error)
{
  int c;	/* Character of the parsed option.  */

  int error_occurred = 0;
  struct create_object_args local_args_info;
  ...
  if (initialize)
    create_object_parser_init (args_info);

  create_object_parser_init (&local_args_info);
  ...
  while (1)
    {
      ...
      c = custom_getopt_long (argc, argv, "ho:i:", long_options, &option_index);
      ...
      switch (c)
        ...
        case 'h':	/* Print help and exit.  */
          ...
          if (update_arg( 0 , 
              ...
              additional_error))
            goto failure;
          create_object_parser_free (&local_args_info);
          return 0;
        ...
        default:	/* bug: option not considered.  */
          fprintf (stderr, "%s: option unknown: %c%s\n", CREATE_OBJECT_PARSER_PACKAGE, c, (additional_error ? additional_error : ""));
          abort ();
        } /* switch */
    } /* while */
  ...
  if (check_required)
    {
      error_occurred += create_object_parser_required2 (args_info, argv[0], additional_error);
    }

  create_object_parser_release (&local_args_info);
...
static unsigned int
create_object_parser_create_argv(const char *cmdline_, char ***argv_ptr, const char *prog_name)
{
  char *cmdline, *p;
  size_t n = 0, j;
  int i;
...
int
create_object_parser_string(const char *cmdline, struct create_object_args *args_info, const char *prog_name)
{
  return create_object_parser_string2(cmdline, args_info, prog_name, 0, 1, 1);
}

int
create_object_parser_string2(const char *cmdline, struct create_object_args *args_info, const char *prog_name,
    int override, int initialize, int check_required)
{
  ...
  return create_object_parser_string_ext(cmdline, args_info, prog_name, &params);
}

int
create_object_parser_string_ext(const char *cmdline, struct create_object_args *args_info, const char *prog_name,
    struct create_object_parser_params *params)
{
  char **argv_ptr = 0;
  int result;
  ...
  argc = create_object_parser_create_argv(cmdline, &argv_ptr, prog_name);
  
  result =
    create_object_parser_internal (argc, argv_ptr, args_info, params, 0);
