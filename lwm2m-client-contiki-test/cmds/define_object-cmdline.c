/*
  ...
  gengetopt -i cmds/define_object.cmdline --string-parser --include-getopt --no-handle-error --no-handle-help --no-version --func-name=define_object_parser --arg-struct-name=define_object_args --file-name=cmds/define_object-cmdline 
...
const char *define_object_args_purpose = "";

const char *define_object_args_usage = "Usage: define_object [OPTIONS]...";

const char *define_object_args_versiontext = "";

const char *define_object_args_description = "";

const char *define_object_args_help[] = {
  "  -h, --help          Print help and exit",
  "  -o, --objectID=INT  Object ID",
  "  -n, --min=INT       Minimum number of Object instances",
  "  -m, --max=INT       Maximum number of Object instances",
    0
};

typedef enum {ARG_NO
  , ARG_INT
} define_object_parser_arg_type;

static
void clear_given (struct define_object_args *args_info);
static
void clear_args (struct define_object_args *args_info);

static int
define_object_parser_internal (int argc, char **argv, struct define_object_args *args_info,
                        struct define_object_parser_params *params, const char *additional_error);

static int
define_object_parser_required2 (struct define_object_args *args_info, const char *prog_name, const char *additional_error);
struct line_list
{
  char * string_arg;
...
static
void init_args_info(struct define_object_args *args_info)
{


  args_info->help_help = define_object_args_help[0] ;
  args_info->objectID_help = define_object_args_help[1] ;
  args_info->min_help = define_object_args_help[2] ;
  args_info->max_help = define_object_args_help[3] ;
  
}

void
define_object_parser_print_version (void)
{
  printf ("%s %s\n",
     (strlen(DEFINE_OBJECT_PARSER_PACKAGE_NAME) ? DEFINE_OBJECT_PARSER_PACKAGE_NAME : DEFINE_OBJECT_PARSER_PACKAGE),
     DEFINE_OBJECT_PARSER_VERSION);

  if (strlen(define_object_args_versiontext) > 0)
    printf("\n%s\n", define_object_args_versiontext);
}

static void print_help_common(void) {
  define_object_parser_print_version ();

  if (strlen(define_object_args_purpose) > 0)
    printf("\n%s\n", define_object_args_purpose);

  if (strlen(define_object_args_usage) > 0)
    printf("\n%s\n", define_object_args_usage);

  printf("\n");

  if (strlen(define_object_args_description) > 0)
    printf("%s\n\n", define_object_args_description);
}

void
define_object_parser_print_help (void)
{
  int i = 0;
  print_help_common();
  while (define_object_args_help[i])
    printf("%s\n", define_object_args_help[i++]);
}

void
define_object_parser_init (struct define_object_args *args_info)
{
...
void
define_object_parser_params_init(struct define_object_parser_params *params)
{
  if (params)
    { 
      params->override = 0;
...
define_object_parser_params_create(void)
{
  struct define_object_parser_params *params = 
    (struct define_object_parser_params *)malloc(sizeof(struct define_object_parser_params));
  define_object_parser_params_init(params);  
  return params;
}
...
static void
define_object_parser_release (struct define_object_args *args_info)
{
...
int
define_object_parser_dump(FILE *outfile, struct define_object_args *args_info)
{
  int i = 0;

  if (!outfile)
    {
      fprintf (stderr, "%s: cannot dump options to stream\n", DEFINE_OBJECT_PARSER_PACKAGE);
      return EXIT_FAILURE;
    }
...
int
define_object_parser_file_save(const char *filename, struct define_object_args *args_info)
{
  FILE *outfile;
  int i = 0;
  ...
  if (!outfile)
    {
      fprintf (stderr, "%s: cannot open file for writing: %s\n", DEFINE_OBJECT_PARSER_PACKAGE, filename);
      return EXIT_FAILURE;
    }

  i = define_object_parser_dump(outfile, args_info);
  fclose (outfile);
...
void
define_object_parser_free (struct define_object_args *args_info)
{
  define_object_parser_release (args_info);
}
...
int
define_object_parser (int argc, char **argv, struct define_object_args *args_info)
{
  return define_object_parser2 (argc, argv, args_info, 0, 1, 1);
}

int
define_object_parser_ext (int argc, char **argv, struct define_object_args *args_info,
                   struct define_object_parser_params *params)
{
  int result;
  result = define_object_parser_internal (argc, argv, args_info, params, 0);

  return result;
}

int
define_object_parser2 (int argc, char **argv, struct define_object_args *args_info, int override, int initialize, int check_required)
{
  int result;
  struct define_object_parser_params params;
...
int
define_object_parser_required (struct define_object_args *args_info, const char *prog_name)
{
  int result = EXIT_SUCCESS;

  if (define_object_parser_required2(args_info, prog_name, 0) > 0)
    result = EXIT_FAILURE;
 ...
 * @param check_ambiguity @see define_object_parser_params.check_ambiguity
 * @param override @see define_object_parser_params.override
...
int update_arg(void *field, char **orig_field,
               ...
               define_object_parser_arg_type arg_type,
...
int
define_object_parser_internal (
  int argc, char **argv, struct define_object_args *args_info,
                        struct define_object_parser_params *params, const char *additional_error)
{
  int c;	/* Character of the parsed option.  */

  int error_occurred = 0;
  struct define_object_args local_args_info;
  ...
  if (initialize)
    define_object_parser_init (args_info);

  define_object_parser_init (&local_args_info);
  ...
  while (1)
    {
      ...
      c = custom_getopt_long (argc, argv, "ho:n:m:", long_options, &option_index);
      ...
      switch (c)
        ...
        case 'h':	/* Print help and exit.  */
          ...
          if (update_arg( 0 , 
              ...
              additional_error))
            goto failure;
          define_object_parser_free (&local_args_info);
          return 0;
        ...
        case 'o':	/* Object ID.  */
          ...
          break;
        case 'n':	/* Minimum number of Object instances.  */
          ...
          break;
        case 'm':	/* Maximum number of Object instances.  */
        ...
        default:	/* bug: option not considered.  */
          fprintf (stderr, "%s: option unknown: %c%s\n", DEFINE_OBJECT_PARSER_PACKAGE, c, (additional_error ? additional_error : ""));
          abort ();
        } /* switch */
    } /* while */
  ...
  if (check_required)
    {
      error_occurred += define_object_parser_required2 (args_info, argv[0], additional_error);
    }

  define_object_parser_release (&local_args_info);
...
static unsigned int
define_object_parser_create_argv(const char *cmdline_, char ***argv_ptr, const char *prog_name)
{
  char *cmdline, *p;
  size_t n = 0, j;
  int i;
...
int
define_object_parser_string(const char *cmdline, struct define_object_args *args_info, const char *prog_name)
{
  return define_object_parser_string2(cmdline, args_info, prog_name, 0, 1, 1);
}

int
define_object_parser_string2(const char *cmdline, struct define_object_args *args_info, const char *prog_name,
    int override, int initialize, int check_required)
{
  ...
  return define_object_parser_string_ext(cmdline, args_info, prog_name, &params);
}

int
define_object_parser_string_ext(const char *cmdline, struct define_object_args *args_info, const char *prog_name,
    struct define_object_parser_params *params)
{
  char **argv_ptr = 0;
  int result;
  ...
  argc = define_object_parser_create_argv(cmdline, &argv_ptr, prog_name);
  
  result =
    define_object_parser_internal (argc, argv_ptr, args_info, params, 0);
